...
function MultiError(errors, message) {
    ...
    this.list.forEach(function(err) {
        ...
        if (utils.isValidationError(err)) this.errors[kind.VALIDATION].push(err);
        if (utils.isSystemError(err)) this.errors[kind.SYSTEM].push(err);
        if (utils.isSyntaxError(err)) this.errors[kind.SYNTAX].push(err);
        if (utils.isDbError(err)) this.errors[kind.DB].push(err);
        if (utils.isAccessError(err)) this.errors[kind.ACCESS].push(err);
        if (utils.isUnknownError(err)) this.errors[kind.UNKNOWN].push(err);
