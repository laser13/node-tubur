...
ApiClient.prototype._checkError = function()
{
    if ( !this.errors || !this.last )
    {
        ...
        this.errors.push(new error.SystemError(null, 'constructor is not called'));
...
ApiClient.prototype.send = function(path, data, fn) {
    ...
    if (this._checkError()) return fn(this.errors);
    ...
    request.post(url, { json: data }, function (error, response, body) {
        ...
        if (error)
        {
            return fn(new packet.SystemError(error, 'Failed connect to API service'));
        }
        ...
        if (response.statusCode != 200)
        {
            ...
            return fn(new packet.SystemError(null, 'API service return error: [ http code: ' + response.statusCode + ': ' + message + ' ]'));
        }
