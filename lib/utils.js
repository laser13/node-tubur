...
function isObject(val) { return val != null && val.constructor === Object; /*return (Object.prototype.toString.call(val) === '[object Object]')*/ }
    ...
    for(var i = 0, len = args.length; i < len; i++) {
        ...
        if (isArray(key)) {
            key.forEach(function(k) {
                if ( obj == null || isUndefined(obj[k]) ) return false;
            });
        }
        else {
            if ( obj == null || isUndefined(obj[key]) ) return false;
            obj = obj[key];
        }
    }
...
function isCustomError(err, type) {
    return isInstanceOf(err, error.CustomError) && err.type === type;
}
