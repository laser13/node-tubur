    ...
    return function() {
        ...
        if (options.output === 'mongo' && options.mongoUrl) {
            ...
            MongoClient.connect(options.mongoUrl, {}, function(err, db) {
                ...
                if (str instanceof Error) data['stack'] = str.stack;
    ...
    if ( !utils.hasKey(options, 'scribeSet') ) {
        ...
        if (options.colored) {
            defaultScribeSet = [
                ...
                new ScribeStruct({ name: 'error', options: new ScribeOptStruct({ showHead: true, headTitle: '[ ERROR ]', color: 'red', colored: options.colored, output: options.output, path: options.path, mongoUrl: options.mongoUrl, collectionName: 'error', fileName: 'error', showCodeLine: true }).toObject() }).toObject(),
            ...
            defaultScribeSet = [
                ...
                new ScribeStruct({ name: 'error', options: new ScribeOptStruct({ showHead: true, headTitle: '[ ERROR ]', colored: options.colored, output: options.output, path: options.path, mongoUrl: options.mongoUrl, collectionName: 'error', fileName: 'error', showCodeLine: true }).toObject() }).toObject(),
                new ScribeStruct({ name: 'add', options: new ScribeOptStruct({ showHead: false, colored: options.colored, output: options.output, path: options.path, mongoUrl: options.mongoUrl, collectionName: 'info', fileName: 'info' }).toObject() }).toObject()
            ];
        }
