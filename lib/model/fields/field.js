...
function Field() {
    ...
    this.options = options || { required: optionKeys[opt.key.REQUIRED].default, type: optionKeys[opt.key.TYPE].default };

    // Смотрим было ли передано имя поля
    if (this.name) this._cookOptions();
}
...
//    this.errors.forEach(function(err) { console.log(err.name); });
    return this.errors.length ? new terror.MultiError(this.errors, 'Field validation errors') : null;
};
...
Field.prototype._typeValidator = function(value) {
    ...
    else if (fieldType === 'mixed') {
        checkType = function(val) { return true };
    }
