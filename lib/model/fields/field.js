...
Field.prototype.verify = function(value)
{
    ...
    else if (baseClass.has(fieldType))
    {
        var error = false;
    ...
    else
    {
        ...
        if (isArray)
        {
            value.forEach(function(item) {
                ...
                if ( !(value instanceof klass) )
                {
                    self.errors.push(new appError.ValidationError({ field: fieldName+'['+value.indexOf(item)+']', value: item }, self._cookMsg(msg['instance'], klass, fieldName)));
                    error = true;
                }
            });
            ...
            if ( !error )
            {
                value = value.map(function(item) { return new klass(item) });
            }
        ...
        else
        {
            if ( !(value instanceof klass) )
            {
                self.errors.push(new appError.ValidationError({ field: fieldName, value: value }, self._cookMsg(msg['instance'], klass, fieldName)));
