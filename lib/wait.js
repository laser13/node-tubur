...
Wait.prototype._launch = function(callback) {
    ...
    if ( this.jobs.length == 0 ) {
        var err = (self.errors.length) ? new terror.MultiError(self.errors, 'The task will fail') : null;
        return callback(err);
    }
    ...
    (function loop() {
        setTimeout(function() {
            ...
            if (self.stopKey) {
                keys = [];
                self.jobs = [];
                if (self.errors.length === 0) self.errors.push(new terror.SystemError(null, 'поступил STOP сигнал'));
            }
            // Превысили время выполнения
            if ( self.options.vita && ((currTime - self.startTime) > self.options.vita) ) {
                keys = [];
                self.errors.push(new terror.SystemError(null, 'превышено время ожидания ' + (self.options.vita / 1000) + ' сек.'));
            }
