...
Wait.prototype._launch = function(callback)
{
    ...
    (function loop() {
        ...
        setTimeout(function() {
            ...
            if (_wait.stopKey)
            {
                return callback(new error.SystemError(null, 'поступил STOP сигнал'));
            }
            ...
            {
                keys = [];
                return callback(new error.SystemError(null, 'превышено время ожидания ' + (_wait.options.vita / 1000) + ' сек.'));
            }
